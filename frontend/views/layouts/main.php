<?php

/* @var $this \yii\web\View */

/* @var $content string */

use common\widgets\Alert;
use frontend\assets\AppAsset;
use yii\helpers\Html;
use yii\helpers\Url;

AppAsset::register($this);
?>
<?php $this->beginPage() ?>
<!DOCTYPE html>
<html lang="<?= Yii::$app->language ?>">
<head>
    <meta charset="<?= Yii::$app->charset ?>">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <?= Html::csrfMetaTags() ?>
    <title><?= Html::encode($this->title) ?></title>
    <?php $this->head() ?>
    <?= \Yii::$app->cache->getOrSet('site_js_counters', function () {
        return \modules\config\models\Config::getValue('site_js_counters');
    }, 60 * 60); ?>
</head>
<body>
<?php $this->beginBody() ?>
<div class="hide">

    <svg xmlns="http://www.w3.org/2000/svg">
        <symbol id="icon-arrow-right-long" viewBox="0 0 32 19.41">
            <g>
                <path d="M22.71,19.41,21.29,18l8.29-8.29L21.29,1.41,22.71,0l9,9a1,1,0,0,1,0,1.41Z"/>
                <path d="m0 8.71h31v2h-31z"/>
            </g>
        </symbol>
        <symbol id="icon-arrow-right" viewBox="0 0 96 96">
            <path d="m12 52h62.34l-21.456 21.456c-1.562 1.562-1.562 4.095-.0001 5.656 1.562 1.562 4.096 1.562 5.658 0l28.28-28.28c.186-.186.352-.391.498-.609.067-.101.114-.21.172-.315.066-.124.142-.242.195-.373.057-.135.089-.275.129-.415.033-.111.076-.217.099-.331.052-.26.079-.522.079-.785 0-.003-.0001-.006-.0001-.009-.0001-.259-.027-.519-.078-.774-.024-.12-.069-.231-.104-.349-.039-.133-.069-.268-.123-.397-.058-.139-.136-.265-.208-.396-.054-.098-.097-.198-.159-.292-.146-.221-.314-.427-.501-.614l-28.28-28.28c-1.562-1.562-4.095-1.562-5.657-.0001-1.562 1.562-1.562 4.095 0 5.658l21.456 21.455h-62.34c-2.209 0-4 1.791-4 4 0 2.209 1.791 4 4 4"/>
        </symbol>
        <symbol id="icon-attachment" viewBox="0 0 62.5 87.51">
            <g>
                <path d="M40.63,87.51H21.88A21.9,21.9,0,0,1,0,65.63V41.42H6.25V65.63A15.64,15.64,0,0,0,21.88,81.26H40.63A15.64,15.64,0,0,0,56.25,65.63V21.88A15.64,15.64,0,0,0,40.63,6.25V0A21.89,21.89,0,0,1,62.5,21.88V65.63A21.9,21.9,0,0,1,40.63,87.51Z"/>
                <path d="M35.42,71.78H27.08A11.47,11.47,0,0,1,15.63,60.33V22c0-10.67,5.52-22,15.75-22h9.25V6.25H31.38c-5.95,0-9.5,8-9.5,15.76V60.33a5.21,5.21,0,0,0,5.21,5.21h8.34a5.21,5.21,0,0,0,5.21-5.21V25.4h6.25V60.33A11.47,11.47,0,0,1,35.42,71.78Z"/>
            </g>
        </symbol>
        <symbol id="icon-comment" viewBox="0 0 64.83 68.07">
            <g>
                <path d="M26.94,68.07,10,54.67a10.61,10.61,0,0,1-7.39-3A7.86,7.86,0,0,1,.25,46.11l.06-20c0-5.18,5-8.28,9.69-8.28H45.28c5.19,0,8.4,5.13,8.4,9.88V46.57c0,4.81-3.3,7.92-8.4,7.92H26.94ZM10,24.5c-1.39,0-3,.71-3,1.64l-.06,20a1.14,1.14,0,0,0,.42.85A4,4,0,0,0,10,48h2.4l7.86,6.25V47.83h25c1.73,0,1.73-.47,1.73-1.25V27.72c0-1.66-1-3.21-1.73-3.21Z"/>
            </g>
        </symbol>
        <symbol id="icon-download" viewBox="0 0 80.72 84.47">
            <g>
                <path d="m59.37 84.47h-59.37v-84.47h59.37v18.98h-6.25v-12.73h-46.87v71.97h46.87v-9.25h6.25v15.5"/>
                <path d="m15.73 23.82l-3.94-1.18v-1.19h9.06a11.22 11.22 0 0 1 6.62 1.74 12.73 12.73 0 0 1 4 5l8.67 18.12 4.49-11.5-5.27-10.99-3.94-1.18v-1.19h9.06a11.18 11.18 0 0 1 6.62 1.74 12.75 12.75 0 0 1 4 5l8.66 18.12 8.29-21.31-5.91-2.36v-1.19h14.58v1.19l-6.3 2.36-14.57 36.63h-2.37l-11.66-24.26-9.6 24.26h-2.37l-18.12-37.81"/>
            </g>
        </symbol>
        <symbol id="icon-eye" viewBox="0 0 54.87 53.33">
            <g>
                <path d="M27.44,37.77a11.11,11.11,0,0,0,11.1-11.11v-.14a4.61,4.61,0,1,1-3.35-7.8,11.11,11.11,0,1,0-7.75,19.05Z"/>
                <path d="M48.92 29.37A24.07 24.07 0 0 0 27.43 16.71 24.07 24.07 0 0 0 5.95 29.37L0 26.27A30.72 30.72 0 0 1 27.43 10 30.72 30.72 0 0 1 54.83 26.27"/>
                <path d="M27.43 46.66A30.49 30.49 0 0 1 0 29.29l6-2.94a23.68 23.68 0 0 0 42.89 0l6 2.94A30.49 30.49 0 0 1 27.43 46.66"/>
            </g>
        </symbol>
        <symbol id="icon-favorite" viewBox="0 0 135.46 135.46">
            <path stroke-width="16.93" stroke-miterlimit="22.926" d="m33.86 8.46h67.73v110.07l-33.86-25.4-33.87 25.4z"/>
        </symbol>
        <symbol id="icon-reconcept" viewBox="0 0 96 29">
            <g fill-rule="evenodd">
                <path d="m84.24 15.177c-.089.147.5 1.184.791 1.227.303-.053.643-.103 1.012-.147-.813 1.229-1.507 2.295-1.862 2.903-.77 1.312-1.07 3.636 1.333 3.643.407.0001.886-.048 1.426-.226 1.689-.478 2.836-1.032 4.314-.919.229-.175.66-.807.394-.931-1.041-.066-2.02-.17-4.329.499-.445.144-1.372.372-.764-.973.271-.66 1.389-2.379 2.645-4.259 2.229-.117 4.573-.129 5.89-.018.094-.012.462-.547.395-.573-1.021-.278-3.135-.627-5.389-.743.901-1.34 1.79-2.657 2.419-3.637.103-.383-1.404-1.944-1.858-1.682-.655 1.019-1.988 2.99-3.297 4.942.343.078.825.191 1.396.337-1.653-.008-3.28.138-4.516.557"/>
                <path d="m70.46 18.51c1.02-.013 2.343-.438 2.76-1.208.116-.215-.493-.766-.562-.741-.536.198-1.455 1.026-2.198 1.949m-17.695-6.881l.018-.025c.267-.362 1.786 1.387 1.626 1.675l-1.188 1.942c-.701.453-1.802 1.47-2.434 2.341.569-.643 2.31-1.641 3.481-2.097.785-.277 1.576-.422 2.342-.389.397.018 1.838 1.174 1.66 1.481-1.354 2.22-4.194 5.071-1.589 4.373.935-.393 2.01-.826 2.741-1.315.116-.555.372-1.163.795-1.812 1.914-2.936 4.196-3.728 5.899-3.297.347.116.71.343 1.09.681.047-.052.085-.085.155-.096.175-.028.569.057.569.057-.373.938-1.416 2.708-2.253 3.106-.196.104-1.047-.934-.934-1.126.321-.407.735-.485.84-.752-.027-.112-.325-.459-.425-.454-.495.135-1.738 1.587-2.451 2.589-.726 1.019-1.077 1.91-.35 2.758.248.431 1.273.049 1.67-.125.963-.423 1.891-.846 2.8-1.257.207-.767.415-1.11 1.071-2.093 1.642-2.459 4.657-4.118 6.391-2.548.777.704.762 1.297.274 2.031-.996 1.5-3.17 2.278-4.66 1.997-.243.367-.402.676-.513.998-.211.602-.144.942.129 1.086.542.286 2.841-.396 5.716-1.407l.783-1.222.599-.953c.185-.256 1.347.416 1.646 1.604l-.528.884c-.085.32-.052.653.134.852.572.613 1.459-.771 2.28-2 .746-1.115 1.094-1.961-.24-2.318-.642-.151-1.803-.06-2.755.094-.193.033-.584-.407-.594-.622.793-.541 2.239-.982 3.494-1.086 2.349.075 4.179 1.746 2.782 4.164-1.552 2.688-3.615 3.457-4.924 3.463-.787.003-1.062-.591-1.258-1.228l-.102.659-3.373 5.647c-.225-.113-2.288-1.061-1.543-2.328l2.716-4.234c-1.857.748-3.844 1.398-5.04 1.469-.935.056-1.726-.163-2.079-.535-.247-.26-.398-.56-.477-.883.119-.137.324-.351.639-.654-1.413.737-2.859 1.387-4.335 1.989-1.61.354-2.984-.204-3.501-1.342 0-.046.241-.344.798-.881-1.408.733-3.315 1.704-4.792 2.1-2.234.598-3.739-.716-2.956-1.877l1.739-2.579c.506-.875.356-.913-.44-.519-1.068.569-3 1.781-4.392 3.654l-.86 1.406c-.137.21-2.174-1.572-1.876-1.936l4.01-6.145c.289-.384.032-.614-.423-.413-.79.35-2.213.935-3.679 1.426.367 1.222-.064 2.622-.895 3.868-1.508 2.26-3.479 3.365-5.185 3.341-1.764-.024-2.704-1.725-2.401-3.358-.164.357-.311.701-.44 1.03-1.624.827-3.459 1.61-5.078 2.187-2.01.318-3.854.133-5.01-.919-.521-.476-.85-1.041-1.013-1.673.145-.189.367-.491.658-.918-1.466 1.055-6.156 3.668-7.559 3.629-1.881-.053-2.926-1.866-2.382-3.627-.113-.026-.226-.054-.338-.086-.626-.091-1.184-1.848.461-1.174.117.054.245.102.38.143.231-.448.464-.853.697-1.214 2.817-4.381 5.636-2.672 6.436-1.56.133.024.247.069.314.136.325.322.027 1.233-.258 1.582-.792.971-2.8 2.053-5.061 2.302-.639 1.377-.387 2.026.501 1.826 1.427-.321 4.053-1.705 6.076-2.79.195-1.435.938-3.054 2.02-4.689 4.898-7.389 12.952-12.511 15.597-10.09.416.38.673.964.743 1.518 0 .006.158-.283.194-.871.932-.513 1.796-.681 2.327-.69.575-.018 1.936.304 1.526.654-1.359.002-2.669 1.828-3.698 3.379-.489.739-.981 1.472-1.478 2.205-1.455-.389-2.188-1.604-1.904-2.096.375-.563.734-1.139 1.154-1.67.021-.321-.099-.534-.294-.711-2.09-1.892-16.372 11.434-12.547 15.07.694.66 1.506.829 2.422.839 2.335-.663 4.343-1.603 5.799-2.429.382-.82 1.166-1.684 2.138-1.766.376-.031 1.314.623 1.097.987-.288.257-.934.756-1.341 1.03-.74.82-.647 1.677-.167 2.111.749.678 3.793-2.371 3.793-4.281.216-.181.466-.417.749-.711-2.191 1.072-4.49.235-3.588-1.193.974-1.542 3.511-1.653 4.744-.592 2.79-.635 6.386-2 6.885-2.705m-32.12 6.555c1.673-.236 3.269-.986 3.408-2.177-1.302.216-2.537.952-3.408 2.177"/>
                <path d="m.509 11.462c-.044.117 1.084.132 1.461-.113 3.855-3.553 13.817-10.457 17.701-6.862.865.801 1.418 2.841-.315 5.454-1.413 2.13-3.769 4.069-6.335 5.465l-.023-.061c-.143-.632-2.223.256-2.344 1.168-.517.203-.881.257-1.417.314l4.605-7.617c.951-1.646.974-1.731-.229-1.8-.636-.036-1.428-.167-2.376 1.477l-7.734 11.989c-1.288 2.124 1.748 1.857 2.172 1.857l1.784-2.954c-.44-.184-.782-.408-1.119-.595 1.935.187 2.897.123 4.756-.571 1.196 3.084 4.566 6.795 7.509 8.08.621.248 1.707.209 1.451-.092-4.314-4.338-5.422-6.73-6.326-9.221 3.301-1.821 6.526-4.594 7.904-6.75 2.318-3.627 2.191-6.142.401-7.817-5.395-5.05-17.781 3.83-21.526 8.645"/>
            </g>
        </symbol>
        <symbol id="icon-search" viewBox="0 0 139 139">
            <path d="m127.56 111.96l-27.309-27.311c4.64-7.387 7.333-16.12 7.333-25.488 0-26.509-21.49-47.996-47.998-47.996-26.508 0-47.996 21.487-47.996 47.996 0 26.51 21.487 47.995 47.996 47.995 10.197 0 19.642-3.188 27.414-8.605l26.984 26.986c1.875 1.873 4.333 2.806 6.788 2.806 2.458 0 4.913-.933 6.791-2.806 3.747-3.751 3.747-9.827-.003-13.577m-67.974-20.354c-17.917 0-32.443-14.525-32.443-32.443s14.526-32.444 32.443-32.444c17.918 0 32.443 14.526 32.443 32.444s-14.525 32.443-32.443 32.443"/>
        </symbol>
        <symbol id="icon-share-facebook" viewBox="0 0 338.66 338.66">
            <path d="m118.29 139.61v36.46h29.16v88.6h36.46v-88.6h36.45v-36.46h-36.45v-18.5c0-6.18 3.92-11.22 7.29-11.22h29.16v-35.9h-29.16c-24.11 0-43.75 21.32-43.75 47.68v17.94h-29.16"/>
        </symbol>
        <symbol id="icon-share-googleplus" viewBox="0 0 338.66 338.66">
            <path d="m176.93 222.52c0-21.28-21.82-38.19-49.1-38.19h-3.82c-1.09-2.18-1.63-4.91-1.63-8.18 0-2.73.54-8.19 1.63-8.19h1.09c22.37 0 40.92-18 40.92-40.91 0-5.46-2.73-12.01-5.46-15.28l14.19-7.54v-26.83l-29.46 16.19c-6.55-4.27-13.09-6.92-21.28-6.92-22.37 0-40.37 18.1-40.37 40.38 0 13.09 6.01 24 15.28 31.64-2.73 5.45-3.82 11.46-3.82 17.46 0 5.45 1.09 10.91 3.27 15.82-12 7.09-19.64 18-19.64 30.55 0 21.46 21.82 38.73 49.1 38.73 27.28 0 49.1-17.27 49.1-38.73m37.07-40.27v-32.39h-32.22v-13.54h32.22v-32.22h13.71v32.22h32.22v13.54h-32.22v32.39h-13.71m-69.81-55.2c0 10.91-8.72 19.09-19.09 19.09-10.36 0-19.09-8.18-19.09-19.09 0-10.37 8.73-19.09 19.09-19.09 10.37 0 19.09 8.72 19.09 19.09m5.46 95.47c0 8.18-9.82 14.73-21.82 14.73-12 0-21.82-6.55-21.82-14.73 0-8.18 9.82-14.73 21.82-14.73 12 0 21.82 6.55 21.82 14.73"/>
        </symbol>
        <symbol id="icon-share-ok" viewBox="0 0 338.66 338.66">
            <g>
                <path d="m129.14 201.91c3.42 1.99 9.52 4.59 13.31 5.73l5.31 1.58c.86.22 1.96.34 2.74.71-.64.95-23.88 23.93-26.16 26.23-.67.68-1.2 1.54-1.69 2.39-2.76 4.79-2.23 10.94 1.26 15.23.14.17.21.22.36.4 1.12 1.32 2.53 2.42 4.11 3.18 4.16 1.98 9.02 1.87 12.99-.46.91-.53 1.52-1.02 2.27-1.7.07-.06.16-.14.2-.18l1.28-1.27c.02-.02.05-.04.07-.06l4.66-4.67c.001-.02.04-.05.06-.06l1.1-1.08c.09-.1.15-.16.25-.26l5.56-5.54c.91-1.02 11.6-11.9 12.45-12.47.66.18 1.51 1.32 2.01 1.82l21.65 21.66c2.11 2.11 3.97 3.96 6.91 4.96 8.74 3.01 17.95-3.65 17.95-12.21 0-4.19-.97-6.7-3.53-9.75-.39-.46-.53-.65-.96-1.08l-7.94-7.91c-.02-.001-.05-.04-.07-.06l-2.26-2.21c-.001-.02-.04-.05-.06-.07l-2.74-2.74c-.34-.37-.64-.64-.95-.96l-.13-.13-1.4-1.41c-.18-.17-.35-.32-.52-.51-.02-.001-.05-.04-.07-.06l-6.9-6.89c-.02-.02-.05-.04-.07-.06l-.51-.52c-.21-.2-.3-.3-.5-.51-.02-.02-.05-.05-.07-.06l-.47-.42c-.28-.27-.31-.23-.34-.68.92 0 1.86-.37 2.71-.62.95-.26 1.75-.46 2.69-.74 2.57-.75 5.02-1.61 7.48-2.62.77-.32 1.6-.68 2.42-1.02 3.18-1.3 10.7-5.15 13.02-7.3 2.29-2.12 4.49-5.7 4.49-9.04 0-3.64-.2-5-1.91-8.18-.54-.99-1.35-1.93-2.14-2.71l-1.77-1.42c-.69-.46-1.23-.8-2.03-1.17-4.29-1.99-9.02-1.46-13 .92-1.84 1.1-3.86 2.41-5.86 3.22-.69.28-1.34.6-2.02.91-9.61 4.24-21.34 5.82-31.76 4.05l-2.61-.46c-3.27-.56-6.44-1.51-9.43-2.7-.72-.28-1.37-.5-2.12-.82-2.71-1.13-5.45-2.5-7.89-4.1-5.28-3.49-12.63-2.79-17.1 1.69-.3.3-.4.51-.7.84-1.91 2.14-3.22 5.2-3.22 8.27 0 3 .64 5.19 1.91 7.54.22.41.43.67.72 1.07.49.72 1.01 1.24 1.61 1.84.31.3.57.48.9.75 1.87 1.54 4.35 2.68 6.45 3.9"/>
                <path d="m137.82 159.89c1.63 1.57 4.43 3.66 6.32 4.92.32.22.53.35.86.54.3.18.51.36.86.55.64.36 1.2.64 1.82.99 5.51 3.05 13.46 5.36 20.06 5.36 2.84 0 5.36.07 8.15-.41 2.45-.42 4.46-.75 6.8-1.5 2.24-.71 3.98-1.36 6.11-2.31 3.3-1.47 5.89-3.23 8.65-5.28 1.72-1.27 5.31-4.53 6.48-6.04 1.34-1.73 2.92-3.33 4.2-5.5 1.18-2.01 1.96-3.1 3.02-5.41 1.82-3.99 3.27-8.32 3.88-12.87.85-6.48.64-11.98-.94-18.3-.68-2.75-1.79-5.79-3.02-8.35-1.22-2.55-4.52-8.07-6.72-10.28l-1.34-1.47c-.52-.52-.95-.95-1.47-1.47-6.53-6.53-15.18-10.85-24.3-12.48-1.74-.31-6.49-.71-7.84-.71-1.84 0-3.74.27-5.37.38-1.62.11-3.44.49-4.93.82-2.45.55-6.46 1.83-8.6 2.89-.34.16-.61.25-.99.43-2.21 1.02-6.2 3.42-8 4.9-.56.46-1.11.81-1.65 1.29-1.37 1.22-3.15 2.78-4.41 4.14l-1.34 1.6c-.89 1.05-1.75 2.06-2.51 3.23-2.32 3.52-2.12 3.12-4.22 7.28-.65 1.29-1.13 2.7-1.65 4.1-1.57 4.33-2.56 9.72-2.56 14.68 0 5.98.97 12.08 3.2 17.49 2.77 6.7 6.35 11.88 11.45 16.79m30.87-15.08c-2.68 0-5.91-.94-8.14-2.18-.26-.14-.48-.25-.74-.4-.14-.08-.23-.15-.35-.22-.13-.08-.22-.13-.35-.22-.76-.51-1.9-1.36-2.57-2-2.06-1.99-3.52-4.09-4.64-6.81-.9-2.2-1.3-4.67-1.3-7.1 0-2.02.4-4.2 1.04-5.96.21-.57.4-1.14.67-1.67.85-1.69.77-1.53 1.71-2.96.31-.47.66-.87 1.02-1.3l.54-.65c.52-.55 1.24-1.18 1.8-1.68.21-.2.44-.34.67-.53.72-.6 2.34-1.57 3.25-1.99.15-.07.26-.1.39-.17.87-.43 2.5-.95 3.49-1.17.61-.14 1.35-.29 2.01-.34.66-.04 1.43-.15 2.17-.15.56 0 2.48.16 3.19.29 3.7.65 7.21 2.41 9.86 5.07.22.21.39.38.6.59l.55.6c.88.89 2.23 3.13 2.72 4.17.5 1.04.95 2.28 1.23 3.39.64 2.56.72 4.8.37 7.43-.24 1.85-.83 3.6-1.57 5.22-.42.94-.74 1.39-1.22 2.2-.52.88-1.16 1.54-1.71 2.23-.47.61-1.93 1.94-2.63 2.46-1.12.83-2.17 1.54-3.5 2.14-.87.38-1.58.65-2.49.94-.95.3-1.76.44-2.76.61-1.14.19-2.15.16-3.31.16"/>
            </g>
        </symbol>
        <symbol id="icon-share-twitter" viewBox="0 0 338.66 338.66">
            <path d="m58.92 201.57c55.94 76.06 176.65 44.85 194.9-28.27 12.96 0 21.2-4.71 25.91-10.01-7.65 1.18-18.84 0-24.73-2.35 12.37-1.18 20.61-7.07 24.14-14.72-4.71 2.94-18.84 5.88-26.49 2.94 0-1.77-.59-3.53-1.18-4.71-5.89-21.89-25.91-38.96-46.52-37.2l5.3-1.76c1.77-.59 15.31-2.95 13.54-7.66-1.76-4.12-18.25 2.95-21.78 4.12 4.12-1.76 11.18-4.12 11.77-8.83-6.47.59-12.36 3.53-17.07 8.24 1.76-2.35 2.94-4.12 2.94-7.06-16.49 11.19-26.5 32.38-34.15 53.68-6.48-5.89-11.78-10.6-17.08-13.54-13.54-7.17-30.62-15.41-56.53-24.83-.59 8.83 4.12 20.02 18.85 27.77-2.95-.59-8.84.59-13.55 1.77 1.77 10.01 8.25 18.25 24.73 21.79-7.65.59-11.77 2.35-15.31 5.89 3.54 7.06 12.37 15.3 27.68 13.54-17.08 7.65-7.07 21.2 6.48 18.84-23.56 24.24-60.65 22.48-81.85 2.36"/>
        </symbol>
        <symbol id="icon-share-vk" viewBox="0 0 338.66 338.66">
            <path d="m120.89 201.12c21.74 22.65 46.43 21.17 46.43 21.17h11.18c0 0 3.39-.38 5.12-2.24 1.58-1.71 1.53-4.92 1.53-4.92 0 0-.23-15.01 6.75-17.22 6.87-2.18 15.7 14.51 25.04 20.92 7.07 4.86 12.44 3.8 12.44 3.8l25-.34c0 0 13.08-.81 6.88-11.1-.51-.84-3.61-7.6-18.6-21.51-15.67-14.54-13.57-12.19 5.31-37.36 11.51-15.33 16.1-24.68 14.66-28.69-1.37-3.82-9.82-2.81-9.82-2.81l-28.15.18c0 0-2.09-.29-3.64.64-1.51.9-2.48 3.01-2.48 3.01 0 0-4.45 11.87-10.4 21.95-12.53 21.29-17.54 22.42-19.59 21.09-4.77-3.08-3.58-12.38-3.58-18.98 0-20.62 3.14-29.22-6.09-31.45-3.06-.74-5.31-1.23-13.14-1.31-10.04-.1-18.54.04-23.35 2.39-3.2 1.57-5.68 5.06-4.17 5.27 1.86.24 6.07 1.13 8.3 4.17 2.89 3.92 2.79 12.74 2.79 12.74 0 0 1.66 24.28-3.87 27.3-3.8 2.06-9-2.16-20.18-21.47-5.72-9.89-10.04-20.82-10.04-20.82 0 0-.83-2.05-2.32-3.14-1.81-1.32-4.32-1.74-4.32-1.74l-26.75.17c0 0-4.02.11-5.49 1.86-1.31 1.55-.11 4.77-.11 4.77 0 0 20.94 48.99 44.66 73.67"/>
        </symbol>
    </svg>

</div>
<div id="wrapper">

    <div class="container">

        <header id="header" class="header">
            <div class="content content--lg">
                <div class="header-main">
                    <a href="/" class="logo">
                        <img src="/svg/logo.svg" alt="ReConcept">
                    </a>
                    <nav class="nav">
                        <div class="elements">
                            <a href="/blog/64-sajty-5-go-pokoleniya" class="slogan">
                                сайты 5-го поколения
                            </a>
                            <button type="button" class="nav-burger" id="burger">
                                <span></span>
                            </button>
                        </div>
                        <ul class="nav-menu">
                            <li><?= Html::a('О нас', Url::to('/site/about')) ?></li>
                            <li><?= Html::a('Портфолио', Url::to('/portfolio')) ?></li>
                            <!--                            <li><a href="web-development.html" class="">Веб-разработка+</a></li>-->
                            <!--                            <li><a href="advertising.html" class="">Реклама+</a></li>-->
                            <!--                            <li><a href="branding.html" class="">Брендирование+</a></li>-->
                            <li><?= Html::a('Блог', Url::to('/blog')) ?></li>
                            <!--                            <li><a href="/site/about" class="">Мы</a></li>-->
                            <li><a href="/contacts" class="">Контакты</a></li>
                            <?php if (Yii::$app->user->can('adminPanel')): ?>
                                <li><?= Html::a('Админка', Url::to(Yii::$app->params['back'])) ?></li>
                            <?php endif; ?>
                        </ul>
                        <div class="support">
                            <?= \frontend\widgets\favoritesCount\FavoritesCount::widget() ?>
                            <button type="button" id="showSupport" class="questions">Есть вопрос?</button>
                        </div>
                    </nav>
                    <div class="close-mobile-nav"></div>
                    <div class="mobile-elements">
                        <a href="/blog/64-sajty-5-go-pokoleniya" class="slogan">
                            сайты 5-го поколения
                        </a>
                        <button type="button" class="burger" id="mobileBurger">
                            <span></span>
                        </button>
                    </div>
                </div>
            </div>
        </header>
        <?= Alert::widget() ?>
        <?= $content ?>

        <?= \frontend\widgets\footer\Footer::widget() ?>
    </div>
</div>
<?= \frontend\widgets\forms\Forms::widget(['viewName' => 'blog_search']) ?>

<div class="modal support-modal">
    <div class="modal-header">
        <div class="content content--lg">
            <div class="modal-header--actions">
                <a href="/" class="logo">
                    <img src="/svg/logo.svg" alt="ReConcept">
                </a>
                <button class="close">
                    Закрыть
                </button>
            </div>
        </div>
    </div>
    <?= \frontend\widgets\forms\Forms::widget(['viewName' => 'support']) ?>
</div>
<?php $this->endBody() ?>
</body>
</html>
<?php $this->endPage() ?>
